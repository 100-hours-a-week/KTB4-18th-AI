...
| `emb_clap` | vector(512) | **O** | **추천용 임베딩.** NULL이면 아직 처리되지 않은 곡 |
| `model_version` | VARCHAR(128) | O | 벡터를 생성한 모델. 모델 교체 시 재임베딩 대상을 골라내는 데 쓴다 |
...
### 1. `emb_clap IS NULL` 이 곧 처리 대기열이다

사용자가 카탈로그에 없는 곡을 검색하면 **메타데이터만 먼저 INSERT**하고 벡터는 비워둔다. 야간 배치가 채운다.
...
**추천 대상 조회 시에는 반드시 NULL을 제외한다.**

```sql
SELECT ... FROM tracks WHERE emb_clap IS NOT NULL ...
```

이 조건이 "오늘 등록된 곡은 내일부터 추천에 나온다"를 구현한다. 별도 큐 테이블이 없다.
...
**장르로 곡의 성격을 판단하지 않는다.** 표시용, 그리고 사용자가 명시적으로 장르를 지정했을 때의 필터로만 쓴다. 곡의 실제 성격은 `emb_clap`과 `mood_tags`가 담는다.
...
현 규모에서는 **벡터 인덱스(HNSW 등)가 불필요하다.** 전수 스캔이 밀리초 단위이며 ANN 인덱스는 근사값을 반환하므로 오히려 정확도 손해다. 100,000곡을 넘어가면 그때 검토한다.

쓰기는 초기 적재 1회와 야간 배치뿐이고, 이후는 읽기 전용에 가깝다.

---

## 미해결 사항: CLAP을 어디서 돌릴지

현재 `pyproject.toml`은 `torch`/`laion-clap`/`essentia-tensorflow`를 `worker` 그룹에만 두고 API 배포 이미지에는 넣지 않는다(무겁다는 이유). 이는 "API 서버는 검색만 하고, 임베딩은 야간 배치가 전담한다"는 전제였다.

그런데 자연어 질의를 `db.search.search()`로 연결하려면(`backend/recommendation.py`의 TODO 참고) 사용자가 입력한 문장을 CLAP **텍스트** 인코더로 임베딩해야 한다 — 이건 검색 경로이므로 API 서버가 요청 시점에 처리해야 한다. 다음 둘 중 하나를 정해야 의존성 구조가 확정된다.

1. **CLAP 텍스트 인코더를 API 서버에 직접 넣는다.** DB에 없는 곡의 오디오 임베딩(essentia 백본 + CLAP 오디오 인코더)은 그대로 야간 배치 전담. 이 경우도 `torch`/`laion-clap`은 API 배포 이미지에 추가로 들어간다 — 지금처럼 API 이미지가 ML 의존성과 완전히 무관하진 않게 된다. 다만 essentia와 그 모델 가중치는 여전히 worker 전용으로 남길 수 있다.
2. **오디오/텍스트를 함께 받는 외부 임베딩 API를 쓴다.** API 서버는 로컬에 무거운 ML 라이브러리를 설치하지 않고 얇게 유지된다.

2번이 아니라면 결국 CLAP을 쓰는 것이고, CLAP을 쓴다면 텍스트 인코더는 API 서버 요청 경로에 있어야 한다 — 그 경우 `worker` 그룹에만 몰아둔 지금의 구성은 최소한 부분적으로는 다시 나눠야 한다. 결정 후 `pyproject.toml`의 의존성 그룹 구성을 다시 봐야 한다.